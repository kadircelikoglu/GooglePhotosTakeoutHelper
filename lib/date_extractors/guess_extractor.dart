...
final List<List<Pattern>> _commonDatetimePatterns = [
  // example: Screenshot_20190919-053857_Camera-edited.jpg
  [RegExp(r'(?<date>(20|19|18)\d{2}(01|02|03|04|05|06|07|08|09|10|11|12)[0-3]\d-\d{6})'), 'YYYYMMDD-hhmmss'],
  ...
  [
    RegExp(r'(?<date>(20|19|18)\d{2}-(01|02|03|04|05|06|07|08|09|10|11|12)-[0-3]\d-\d{2}-\d{2}-\d{2})'),
    'YYYY-MM-DD-hh-mm-ss',
  ],
  // example: signal-2020-10-26-163832.jpg
  [
    RegExp(r'(?<date>(20|19|18)\d{2}-(01|02|03|04|05|06|07|08|09|10|11|12)-[0-3]\d-\d{6})'),
    'YYYY-MM-DD-hhmmss',
  ],
  ...
  [
    RegExp(r'(?<date>(20|19|18)\d{2}(01|02|03|04|05|06|07|08|09|10|11|12)[0-3]\d{7})'),
    'YYYYMMDDhhmmss',
  ],
  // example: 2016_01_30_11_49_15.mp4
  [
    RegExp(r'(?<date>(20|19|18)\d{2}_(01|02|03|04|05|06|07|08|09|10|11|12)_[0-3]\d_\d{2}_\d{2}_\d{2})'),
    'YYYY_MM_DD_hh_mm_ss',
  ],
];
...
Future<DateTime?> guessExtractor(File file) async {
  for (final pat in _commonDatetimePatterns) {
    ...
    DateTime? date;
    try {
      date = FixedDateTimeFormatter(pat.last as String, isUtc: false).tryDecode(dateStr);
